related_topics <- function(widget, comparison_item, hl, time) {
  
  i <- which(grepl("topics", widget$title) == TRUE)
  
  res <- lapply(i, create_related_topics_payload, widget = widget, hl = hl, period = time)
...
create_related_topics_payload <- function(i, widget, hl, period) {
